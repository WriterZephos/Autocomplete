...
public class AutocompleteGUI extends JFrame {
    ...
    private static final String searchURL = "https://www.google.com/search?q=";
                ...
                if (Math.min(k, allResults.length) > 0) {
                    for (int i = 0; i < results.length; i++) {
                        ...
                        results[i] = "<html><table width=\"" 
                                ...
                                + "<tr><td align=left>" 
                                + query.substring(0, (textLen < query.length()) ? textLen + 1: query.length())
                                + "<b>" + query.substring((textLen < query.length()) ? textLen + 1 : query.length()) + "</b>";
                        if (displayWeights) {
    ...
    private void searchOnline(String s) {
        ...
        try {
            URI tempAddress = new URI(searchURL + URLEncoder.encode(s.trim(), "UTF-8"));
